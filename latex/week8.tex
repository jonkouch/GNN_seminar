...
\begin{frame}{\emph{DeepSets} Layer}
    ...
    \begin{proof}[Proof (through example)]
        Let $x={x_1,x_2,x_3}$ be a set, $W \in \mathbb{R}^{3 \times 3}$, and $L(x) = W\mathbf{x}$ such that $L$ is equivariant. W = $\begin{pmatrix}
            a & b & c\\
            d & e & f\\
            g & h & i
        \end{pmatrix}$
        %\pause
        Regard permutation $g=(2,3) \in S_3$. Note that $\left(W\begin{pmatrix}
            x_1\\
            x_2\\
            x_3
...
\begin{frame}{\emph{DeepSets} Layer}
    \begin{proof}[Proof (through example) - continued]
        $ax_1 + bx_2 + cx_3 = ax_1 + bx_3 + cx_2$. This should hold for any choice of $x_1,x_2,x_3$. $\Rightarrow b = c$. Similarly, $d=f, g=h$. \\
        \pause
        ...
        Intuitively, looking at all possible permutations, we can see why all values in the diagonal must be equal and all values in the off-diagonal must be equal. That means $W = \begin{pmatrix}
            a & b & b\\
            b & a & b\\
            b & b & a
        \end{pmatrix}$.
...
\begin{frame}{Invariant and Equivariant Graph Networks}
    \begin{itemize}
        ...
        \item Once again converting to a vector form, we get:
